namespace UI{
	...
	bool Init() {
		// load Bokor font
		if (!fonts["Bokor"].openFromFile("res/Fonts/Bokor.ttf")) {
			std::cout << "RES ISSUE : Couldn't Load Font 'Bokor'" << std::endl;
			return false;
		}

		// load Bokor font
		if (!fonts["SUSE"].openFromFile("res/Fonts/SUSE.ttf")) {
			std::cout << "RES ISSUE : Couldn't Load Font 'SUSE'" << std::endl;
			return false;
		}

		text = new sf::Text(fonts["SUSE"]);
	...
	void DrawText(std::string txt, int x, int y, std::string font="SUSE", int font_size=24, sf::Color font_color=sf::Color::Black, float align_x=Align::Left, float align_y=Align::Top) {
		...
		text->setPosition(sf::Vector2f(x + bounds.size.x * align_x, y + bounds.size.y * align_y));
