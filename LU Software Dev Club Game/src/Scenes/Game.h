...
public:
	...
	virtual void Init() {
        ...
        if (!player_texture->loadFromFile("res/Textures/Player.png")) {
            std::cout << "RES ISSUE : Couldn't Load Texture 'Player.jpg'" << std::endl;
            return;
        }

        player = new sf::Sprite(*player_texture);
        ...
        if (!zombie_texture->loadFromFile("res/Textures/Zombie.png")) {
            std::cout << "RES ISSUE : Couldn't Load Texture 'Zombie.jpg'" << std::endl;
            return;
        }

        zombie = new sf::Sprite(*zombie_texture);
        zombie->setPosition(sf::Vector2f(0.0f, 0.0f));
        zombie->setScale(sf::Vector2f(1.0f / zombie_texture->getSize().x * 100, 1.0f / zombie_texture->getSize().y * 100));

        bullet_texture = new sf::Texture();
        if (!bullet_texture->loadFromFile("res/Textures/Bullet.png")) {
            std::cout << "RES ISSUE : Couldn't Load Texture 'Bullet.png'" << std::endl;
            return;
        }

        bullet_sprite = new sf::Sprite(*bullet_texture);
        bullet_sprite->setScale(sf::Vector2f(0.5f / bullet_texture->getSize().x * 50, 0.5f / bullet_texture->getSize().y * 50));
	}
	...
	virtual void Update() {
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Space) && time_till_shoot <= 0.0f) {
            Bullet bullet;
            bullet.x = x+90;
            bullet.y = y+35;
        ...
        for (Zombie& zombie_data : zombies) {
            zombie->setPosition(sf::Vector2f(zombie_data.x, zombie_data.y));
            main_window->Draw(*zombie);
        }
